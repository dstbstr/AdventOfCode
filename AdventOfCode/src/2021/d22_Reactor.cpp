...
SOLUTION(2021, 22) {
    class Cube {
    public:
        constexpr Cube(Vec3<s32> min, Vec3<s32> max, bool isOn) :Min(min), Max(max), On(isOn) {
            Area = static_cast<size_t>(Max.X - Min.X + 1) * static_cast<size_t>(Max.Y - Min.Y + 1) * static_cast<size_t>(Max.Z - Min.Z + 1);
        }
        ...
        constexpr Cube GetIntersectionCube(const Cube& other) const {
            Vec3<s32> min;
            Vec3<s32> max;
            ...
            return Cube(min, max, true);
        }
        constexpr void Subtract(const Cube& other) {
            auto intersection = GetIntersectionCube(other);
            if (!intersection.IsValid()) return;

            for (const auto& cube : Removed) {
                intersection.Subtract(cube);
            }

            Area -= intersection.Area;
            Removed.push_back(intersection);
        }
    ...
    constexpr Cube ParseLine(std::string_view line) {
        ...
        auto yRange = Constexpr::Split(s2[1], "..");
        auto zRange = Constexpr::Split(s2[2], "..");

        Vec3<s32> min;
        Vec3<s32> max;

        Constexpr::ParseNumber(xRange[0].substr(2), min.X);
        Constexpr::ParseNumber(xRange[1], max.X);

        Constexpr::ParseNumber(yRange[0].substr(2), min.Y);
        Constexpr::ParseNumber(yRange[1], max.Y);

        Constexpr::ParseNumber(zRange[0].substr(2), min.Z);
        Constexpr::ParseNumber(zRange[1], max.Z);
    ...
    constexpr bool IsInRange(const Cube & r) {
        ...
        s32 max = std::max(std::max(r.Max.X, r.Max.Y), r.Max.Z);
        return min >= -50 && max <= 50;
    }
    constexpr bool AcceptAll(const Cube&) {
        return true;
    }
